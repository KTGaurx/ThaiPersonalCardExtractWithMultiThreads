...
class PersonalCard:
    ...
    def __extractSep(self, side: Card = Card.FRONT_TEMPLATE, num = 0):
        ...
        if str(self.provider) == Provider.DEFAULT.value:
            if str(box[num]["provider"]) == str(str(Provider.EASYOCR)):
                self.cardInfo[str(self.lang)][box[num]["name"]] = " ".join(str.strip(
                    "".join(self.reader.readtext(imgCrop, batch_size=4 ,detail=0, paragraph=False , width_ths=1.0, blocklist=box[num]["blocklist"]))).split())
            elif str(box[num]["provider"]) == str(Provider.TESSERACT):
                self.cardInfo[str(self.lang)][box[num]["name"]] = str.strip(
                    " ".join(pytesseract.image_to_string(imgCrop, lang=box[num]["lang"].split(",")[0],
                                                            config=box[num]["tesseract_config"])
                                .replace('\n', '')
                                .replace('\x0c', '')
                                .replace('-', '')
                                .replace('"', '')
